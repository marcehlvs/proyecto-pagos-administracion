...
namespace pagos_administracion_mvc.Controllers
{
    ...
    // (Alumno.FamiliaUserId). Alumno solo el propio (Alumno.AlumnoUserId). Se valida en la
    // acción, mismo criterio que el resto de los controllers con datos sensibles por rol.
    [Authorize(Roles = "Admin,Familia,Alumno")]
    public class BoletinesController : Controller
    {
        ...
        public BoletinesController(AdministracionDbContext context, UserManager<ApplicationUser> userManager, BoletinService boletinService)
        {
            _context = context;
        ...
        public async Task<IActionResult> Pdf(int alumnoId, int anioLectivo)
        {
            ...
            var pdf = BoletinPdfBuilder.Generar(datos);
